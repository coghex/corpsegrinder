...
freeCreepMemory ∷ SE SpawnEnv (CG Env) Unit
freeCreepMemory = do
...
freeCreepMemoryF ∷ F.Object (F.Object Json) → Array String → SE SpawnEnv (CG Env) Unit
    ...
    case memData of
      ...
      Just md → case (getField md "target") of
                  ...
                  Right t0 → case (getField md "home") of
                    Left err → log'' LogError $ "freeCreepMemory: " <> show err
                    Right h0 → lift $ freeSpawnNHarv t0 h0
    memory ← lift $ asks (_.memory)
    freeCreepMem' creepN
